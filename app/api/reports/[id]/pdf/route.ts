/**
 * GET /api/reports/[id]/pdf — descarga el PDF ejecutivo del reporte.
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const buffer = await buildReportPdf(report.snapshot);
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${downloadFilename(report, "pdf")}"`,
